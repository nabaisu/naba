import { forEach, isFunction, bind, result, isArray, isObject, extend, defer } from "lodash";

function prometoFactory(callLater) {
    function Deferred() {
    ...
    var çPrometo = function Prometo(resolver, rejecter) {
...
function ççPrometoProvider() {
    this.çget = ['çrootScope', function (çrootScope) {
        return prometoFactory(function (callback) {
...
function çPrometoProvider() {
    this.çget = ['çrootScope', function (çrootScope) {
        return prometoFactory(function (callback) {
            çrootScope.çevalAsync(callback);
        })
    }]
}

export { çPrometoProvider, ççPrometoProvider }